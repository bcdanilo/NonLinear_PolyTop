...
%-------------------------------------------------------------------------%
function [sigma, D, Psi] = material_model(MatModel,MatCase,MatParam,e) 
...
switch MatModel
  case 'Bilinear'
    ...
    g = zeros(nE,1);  % trace of the strain tensor of each element
    g2 = zeros(nE,1); % trace of the square strain tensor of each element
    L = zeros(nE,1); % Lamé parameter of each element
    for ii=1:nE
    if e(1,ii)+e(2,ii)+e33_t(ii)>0 
      g(ii)=e(1,ii)+e(2,ii)+e33_t(ii);
      g2(ii)=e(1,ii)^2+e(2,ii)^2+e33_t(ii)^2;
      L(ii) = Lt;
    else
      g(ii)=e(1,ii)+e(2,ii)+e33_c(ii);
      g2(ii)=e(1,ii)^2+e(2,ii)^2+e33_c(ii)^2;
      L(ii) = Lc;
    end
    end
    E0 = zeros(1,nE); v = E0;
    ...
    Psi = zeros(1,nE); % stored energy density
    Psi(:) = (1/2)*L.*(g.^2) + G*g2;
  case 'Ogden'
    ...
    e3 = 0.*e1;
    J = (1+e1).*(1+e2).*(1+e3);
    e31 = 0.*e1; e32 = 0.*e1; % Derivative of e3 wrt e1 and e2
    %%%%%% PLANE STRESS CONDITION %%%%%%
    if strcmp(MatCase,'PlaneStress')==1
    ...
    % Derivatives of the principal stresses wrt principal strains
    ...
    Psi = sum((mu./alpha).*((1+e1).^(alpha)+(1+e2).^(alpha)+...
        (1+e3).^(alpha)-3),2)+...
        sum((mu./alpha./beta).*(J.^(-alpha.*beta)-1),2);
otherwise
