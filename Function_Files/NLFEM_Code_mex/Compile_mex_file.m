...
if exist('Global_K_compile_mex','file')==3
  ...
  % Create a code generation configuration object to generate a MEX funct
  cfg = coder.config('mex'); % It could also be only cfg = coder.config;
  cfg.GenerateReport = true; % Open the code generation report
  % Reports potential differences between generated code and MATLAB code
  ...
  ARGS{1}{4} = coder.typeof('X',[1 Inf],[0 1]);
  ...
  ARGS{1}{9} = coder.typeof({ARG}, [10  1],[1 0]);
  ARGS{1}{10} = coder.typeof(0,[Inf  2],[1 0]);
