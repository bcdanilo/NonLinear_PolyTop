...
%-------------------------------------------------------------------------%
function [k, f_NL, Psi] = Global_K_compile(ElemNDof,Element,MatModel,...
                                           MatCase,MatParam,Thickness,E,...
                                           W,dNdxi,Node,d)
...
for el = 1:NElem
  ...
  [Ke, Fint_e,Psi(el)] = ...
    LocalK(Node,W,dNdxi,Thickness,NDof,ue,MatModel,MatCase,MatParam,Element{el});
...
function [Ke,Fint_e, Psie] = LocalK(Node,W,dNdxi,Thickness,NDof,ue,...
    MatModel,MatCase,MatParam,eNode)
Ke = zeros(NDof,NDof); Fint_e = zeros(NDof,1); Psie= 0;
nn = length(eNode);
...
for q = 1:length(W) % Loop over Gauss points
  ...
  [S, D_GP, Psi0] = material_model(MatModel,MatCase,MatParam,E);
  Fint_e = Fint_e + B'*S.*WdJ; % Internal force vector
  Ke = Ke + B'*D_GP*B*WdJ; % Tangent stiffness matrix
  Psie= Psie + Psi0*WdJ; % Stored energy
end
